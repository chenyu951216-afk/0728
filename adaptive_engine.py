...
FEATURE_NAMES = ('ret_1', 'ret_4', 'ret_16', 'ema20_gap', 'ema50_gap', 'ema20_slope', 'atr_pct', 'atr_rank', 'adx', 'rsi', 'volume_z', 'range_z', 'wick_ratio', 'dist_vwap_atr', 'bos_up', 'bos_down', 'sweep_low', 'sweep_high', 'fvg_up', 'fvg_down', 'btc_ret_4', 'btc_ret_16', 'eth_btc_rel', 'spot_perp_basis_bps', 'funding', 'oi_change', 'book_imbalance', 'source_agreement_bps', 'hour_sin', 'hour_cos', 'dow_sin', 'dow_cos', 'macro_code', 'phase_code')
...
def build_features(m15: list[dict], h1: list[dict], btc_h1: list[dict], regime: dict[str, Any], extras: dict[str, Any] | None=None) -> dict[str, float]:
    ...
    ranges = [(f(x['h']) - f(x['l'])) / max(f(x['c']), 1e-09) for x in m15[-60:-1]]
    current_range = (f(m15[-1]['h']) - f(m15[-1]['l'])) / max(last, 1e-09)
    body_hi = max(f(m15[-1]['o']), f(m15[-1]['c']))
    body_lo = min(f(m15[-1]['o']), f(m15[-1]['c']))
    wick_ratio = (f(m15[-1]['h']) - body_hi + (body_lo - f(m15[-1]['l']))) / max(f(m15[-1]['h']) - f(m15[-1]['l']), 1e-09)
    flags = microstructure_flags(m15)
    vwap = rolling_vwap(m15)
    btc = [f(x['c']) for x in btc_h1]
    eth_h1 = [f(x['c']) for x in h1]
    btc_ret_4 = btc[-1] / max(btc[-5], 1e-09) - 1 if len(btc) >= 5 else 0
    btc_ret_16 = btc[-1] / max(btc[-17], 1e-09) - 1 if len(btc) >= 17 else 0
    eth_ret_16 = eth_h1[-1] / max(eth_h1[-17], 1e-09) - 1 if len(eth_h1) >= 17 else 0
...
def baseline_strategy_scores(features: dict[str, float], regime: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    sweep_long = clamp(0.35 + 0.3 * features['sweep_low'] + 0.12 * (0.5 - features['rsi']) + 0.08 * max(-features['book_imbalance'], 0), 0, 1)
    sweep_short = clamp(0.35 + 0.3 * features['sweep_high'] + 0.12 * (features['rsi'] - 0.5) + 0.08 * max(features['book_imbalance'], 0), 0, 1)
    squeeze_dir = 1 if features['ret_4'] + features['ema20_slope'] * 0.01 >= 0 else -1
    squeeze_score = clamp(0.3 + 0.2 * (1 - features['atr_rank']) + 0.15 * max(features['volume_z'], 0) + 0.2 * max(features['range_z'], 0), 0, 1)
    breakout_long = clamp(0.25 + 0.32 * features['bos_up'] + 0.13 * max(features['volume_z'], 0) + 0.1 * max(features['oi_change'], 0), 0, 1)
    breakout_short = clamp(0.25 + 0.32 * features['bos_down'] + 0.13 * max(features['volume_z'], 0) + 0.1 * max(features['oi_change'], 0), 0, 1)
    mr_long = clamp(0.3 + 0.16 * max(-features['dist_vwap_atr'], 0) + 0.15 * max(0.45 - features['rsi'], 0) + 0.12 * features['sweep_low'], 0, 1)
    mr_short = clamp(0.3 + 0.16 * max(features['dist_vwap_atr'], 0) + 0.15 * max(features['rsi'] - 0.55, 0) + 0.12 * features['sweep_high'], 0, 1)
...
class ModelStore:
    ...
    def save_challenger(self, strategy: str, model: Any, metrics: dict[str, Any], promote: bool) -> int:
        ...
        self.con.execute('INSERT INTO model_registry(strategy,version,status,created_at,metrics,model) VALUES(?,?,?,?,?,?)', (strategy, version, 'CHAMPION' if promote else 'REJECTED', int(time.time()), json.dumps(metrics, ensure_ascii=False), sqlite3.Binary(pickle.dumps(model, protocol=pickle.HIGHEST_PROTOCOL))))
        self.con.commit()
        return version
...
class Learner:
    ...
    def outcome(cs: list[dict], i: int, direction: str, horizon: int=24) -> tuple[int, float, float, float]:
        ...
        for bar in cs[i + 1:i + 1 + horizon]:
            favorable = (f(bar['h']) - entry) * sign / stop_dist if sign > 0 else (entry - f(bar['l'])) / stop_dist
    ...
    def train_strategy(self, strategy: str, min_train: int=350, min_test: int=140) -> StrategyEvaluation | None:
        ...
        Validation predictions are produced only by models trained on strictly older
        observations. The final deployable model is fitted only after the challenger
        has passed the aggregated OOS guardrails.
        """
        ...
        purge = 32
        n = len(rows)
        first_test = max(min_train + purge, int(n * 0.48))
        remaining = n - first_test
        fold_count = 4 if remaining >= 4 * min_test else 3 if remaining >= 3 * min_test else 2
        if fold_count < 2:
            return None
        fold_size = max(min_test, remaining // fold_count)
        oos_rows: list[dict[str, Any]] = []
        oos_prob: list[float] = []
        fold_stats: list[dict[str, float]] = []
        for fold in range(fold_count):
            test_start = first_test + fold * fold_size
            test_end = n if fold == fold_count - 1 else min(n, test_start + fold_size)
            ...
            model = HistGradientBoostingClassifier(learning_rate=0.045, max_iter=180, max_leaf_nodes=15, min_samples_leaf=28, l2_regularization=1.2, random_state=42 + fold)
            model.fit(x_train, y_train, sample_weight=weights)
            probs = model.predict_proba(x_test)[:, 1]
            chosen = [(r, p) for r, p in zip(test, probs) if p >= 0.58]
            if len(chosen) < 35:
                chosen = list(zip(test, probs))
            pnls = [f(r['pnl_r']) - self.fee_r for r, _ in chosen]
            gains = sum((max(x, 0) for x in pnls))
            losses = sum((max(-x, 0) for x in pnls))
            fold_stats.append({'pf': gains / max(losses, 1e-09), 'ev': mean(pnls), 'win': mean((1.0 if x > 0 else 0.0 for x in pnls)), 'n': float(len(pnls))})
            oos_rows.extend(test)
            oos_prob.extend((float(x) for x in probs))
        if len(fold_stats) < 2 or len(oos_rows) < min_test:
            return None
        y_oos = np.array([r['success'] for r in oos_rows], dtype=int)
        p_oos = np.array(oos_prob, dtype=float)
        if len(set(y_oos)) < 2:
            return None
        ...
        gains = sum((max(x, 0) for x in pnls))
        losses = sum((max(-x, 0) for x in pnls))
        pf = gains / max(losses, 1e-09)
        expectancy = mean(pnls)
        test_win = mean((1.0 if x > 0 else 0.0 for x in pnls))
        train_win = mean((float(r['success']) for r in rows[:first_test - purge]))
        fold_evs = [x['ev'] for x in fold_stats]
        fold_wins = [x['win'] for x in fold_stats]
        ev_dispersion = statistics.pstdev(fold_evs) if len(fold_evs) > 1 else 0.0
        win_dispersion = statistics.pstdev(fold_wins) if len(fold_wins) > 1 else 0.0
        stability = clamp(1.0 - 0.65 * ev_dispersion - 0.55 * win_dispersion, 0, 1)
        brier = brier_score_loss(y_oos, p_oos)
        ll = log_loss(y_oos, p_oos, labels=[0, 1])
        max_dd = self._max_drawdown(pnls)
        worst_fold_ev = min(fold_evs)
        profitable_folds = sum((x['ev'] > 0 for x in fold_stats)) / len(fold_stats)
        ...
        if current_model is not None:
            try:
                old_probs = current_model.predict_proba(np.vstack([_feature_vector(r['features']) for r in oos_rows]))[:, 1]
                old_chosen = [r for r, p in zip(oos_rows, old_probs) if p >= 0.58]
                if len(old_chosen) < max(80, min_test // 2):
                    old_chosen = oos_rows
                old_pnls = [f(r['pnl_r']) - self.fee_r for r in old_chosen]
                old_gains = sum((max(x, 0) for x in old_pnls))
                old_losses = sum((max(-x, 0) for x in old_pnls))
                old_oos_ev = mean(old_pnls)
                old_oos_pf = old_gains / max(old_losses, 1e-09)
                improvement_ok = expectancy >= old_oos_ev - 0.015 and pf >= old_oos_pf * 0.97
            except Exception:
                improvement_ok = False
        core_ok = pf >= 1.16 and expectancy >= 0.07 and (stability >= 0.78) and (brier <= 0.255) and (max_dd <= 18) and (worst_fold_ev >= -0.08) and (profitable_folds >= 0.6)
        promoted = core_ok and improvement_ok
        reason = 'purged walk-forward OOS guardrails passed' if promoted else f'rejected: PF={pf:.2f}, EV={expectancy:.3f}R, worstFold={worst_fold_ev:.3f}R, profitableFolds={profitable_folds:.0%}, stability={stability:.2f}, brier={brier:.3f}, DD={max_dd:.1f}R'
        x_all = np.vstack([_feature_vector(r['features']) for r in rows])
        y_all = np.array([r['success'] for r in rows], dtype=int)
        weights_all = np.array([clamp(r['source_quality'] / 100, 0.5, 1.0) for r in rows])
        final_model = HistGradientBoostingClassifier(learning_rate=0.045, max_iter=180, max_leaf_nodes=15, min_samples_leaf=28, l2_regularization=1.2, random_state=99)
...
def learned_risk_profile(store: ModelStore, strategy: str, regime: str, direction: str) -> dict[str, float]:
    ...
    stop_r = clamp(q(maes, 0.88) + 0.08, 0.72, 1.45)
...
def risk_plan(store: ModelStore, strategy: str, regime: str, direction: str, entry: float, m15: list[dict]) -> dict[str, Any]:
    ...
    risk = abs(entry - stop)
    sign = 1 if direction == 'LONG' else -1
    levels = [profile['tp1_r'], profile['tp2_r'], profile['tp3_r'], profile['runner_r']]
    allocations = [25, 30, 25, 20]
...
def choose_strategy(store: ModelStore, learner: Learner, features: dict[str, float], regime: dict[str, Any], data_quality: float) -> dict[str, Any]:
    baselines = baseline_strategy_scores(features, regime)
    candidates = []
    for strategy, info in baselines.items():
        probability, model_meta = learner.predict(strategy, features, info['baseline'])
        quality_penalty = clamp(data_quality / 100, 0.55, 1.0)
        score = probability * info['affinity'] * quality_penalty
        candidates.append({'strategy': strategy, 'direction': info['direction'], 'baseline': info['baseline'], 'probability': probability, 'score': score, 'model': model_meta})
    candidates.sort(key=lambda x: x['score'], reverse=True)
    best = candidates[0]
    certified = best['model'].get('mode') == 'LEARNED_CHAMPION'
    tradeable = certified and best['probability'] >= 0.6 and (data_quality >= 70) and (best['score'] >= 0.44)
...
def bootstrap_progress(con: sqlite3.Connection) -> dict[str, Any]:
    ...
    for item in details:
        for tf, meta in item['timeframes'].items():
            if meta['from'] is None or meta['to'] is None:
                coverage.append(0.0)
            else:
                coverage.append(clamp((meta['to'] - meta['from']) / max(now - start_2020, 1), 0, 1))
