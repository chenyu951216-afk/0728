...
FEATURE_NAMES = (
    ...
    'funding', 'oi_change', 'book_imbalance', 'liquidation_imbalance',
    'liquidation_intensity', 'oi_weighted_funding', 'taker_imbalance',
    'crowd_skew', 'top_position_skew', 'oi_available', 'funding_available',
    'liquidation_available', 'book_available', 'oi_weighted_funding_available',
    'taker_available', 'crowd_available', 'top_position_available',
    'derivative_coverage', 'derivative_quality', 'source_agreement_bps',
    'hour_sin', 'hour_cos', 'dow_sin', 'dow_cos', 'macro_code', 'phase_code',
...
def build_features(m15: list[dict], h1: list[dict], btc_h1: list[dict], regime: dict[str, Any], extras: dict[str, Any] | None=None) -> dict[str, float]:
    ...
    ddir, h4dir, h1dir = (f(regime.get('daily_direction')), f(regime.get('h4_direction')), f(regime.get('h1_direction')))
    out = {'ret_1': last / max(closes[-2], 1e-09) - 1, 'ret_4': last / max(closes[-5], 1e-09) - 1, 'ret_16': last / max(closes[-17], 1e-09) - 1, 'ema20_gap': (last - e20) / a, 'ema50_gap': (last - e50) / a, 'ema20_slope': (e20 - prior20) / a, 'atr_pct': atr_now, 'atr_rank': percentile_rank(atr_hist, atr_now), 'adx': adx(m15) / 100, 'rsi': rsi(m15) / 100, 'volume_z': clamp(_z(vols, f(m15[-1].get('v'))), -5, 5), 'range_z': clamp(_z(ranges, current_range), -5, 5), 'wick_ratio': clamp(wick_ratio, 0, 1), 'dist_vwap_atr': (last - vwap) / a, **flags, 'btc_ret_4': btc_ret_4, 'btc_ret_16': btc_ret_16, 'eth_btc_rel': eth_ret_16 - btc_ret_16, 'spot_perp_basis_bps': f(extras.get('spot_perp_basis_bps')), 'funding': f(extras.get('funding')), 'oi_change': f(extras.get('oi_change')), 'book_imbalance': f(extras.get('book_imbalance')), 'liquidation_imbalance': f(extras.get('liquidation_imbalance')), 'liquidation_intensity': f(extras.get('liquidation_intensity')), 'oi_weighted_funding': f(extras.get('oi_weighted_funding')), 'taker_imbalance': f(extras.get('taker_imbalance')), 'crowd_skew': f(extras.get('crowd_skew')), 'top_position_skew': f(extras.get('top_position_skew')), 'oi_available': f(extras.get('oi_available')), 'funding_available': f(extras.get('funding_available')), 'liquidation_available': f(extras.get('liquidation_available')), 'book_available': f(extras.get('book_available')), 'oi_weighted_funding_available': f(extras.get('oi_weighted_funding_available')), 'taker_available': f(extras.get('taker_available')), 'crowd_available': f(extras.get('crowd_available')), 'top_position_available': f(extras.get('top_position_available')), 'derivative_coverage': f(extras.get('derivative_coverage')), 'derivative_quality': f(extras.get('derivative_quality')), 'source_agreement_bps': f(extras.get('source_agreement_bps'), 999.0) / 100, 'hour_sin': math.sin(2 * math.pi * tm.tm_hour / 24), 'hour_cos': math.cos(2 * math.pi * tm.tm_hour / 24), 'dow_sin': math.sin(2 * math.pi * tm.tm_wday / 7), 'dow_cos': math.cos(2 * math.pi * tm.tm_wday / 7), 'macro_code': macro_code, 'phase_code': phase_code,
           'daily_direction': ddir, 'h4_direction': h4dir, 'h1_direction': h1dir,
           'daily_adx_norm': f(regime.get('daily_adx')) / 100.0, 'h4_adx_norm': f(regime.get('h4_adx')) / 100.0,
           'h1_adx_norm': f(regime.get('h1_adx')) / 100.0, 'daily_slope_norm': clamp(f(regime.get('daily_slope')) / 5.0, -1, 1),
           'h4_slope_norm': clamp(f(regime.get('h4_slope')) / 5.0, -1, 1), 'h1_slope_norm': clamp(f(regime.get('h1_slope')) / 5.0, -1, 1),
           'macro_alignment': ddir * h4dir, 'structure_alignment': h4dir * h1dir,
           'macro_volatility_rank': f(regime.get('volatility_rank')), 'macro_atr_pct': f(regime.get('h4_atr_pct'))}
