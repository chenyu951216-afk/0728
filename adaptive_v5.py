...
def choose_strategy(store,learner,features,regime,data_quality):
    ...
    for s,dirs in pri.items():
        for d,b in dirs.items():
            p,m=learner.predict(s,d,features,b);meta=m.get('metrics') or {};th=f(meta.get('threshold'),.60);cert=m.get('mode')=='LEARNED_CHAMPION';rg=cert and regime['regime'] in (meta.get('allowed_regimes') or []);allowed_phases=meta.get('allowed_phases') or [];phase_ok=cert and (not allowed_phases or regime['phase'] in allowed_phases);edge=p-th;score=p*aff[s]*qp+clamp(edge,-.2,.2)*.25;trade=bool(cert and rg and phase_ok and data_quality>=70 and p>=th and score>=.42);cand.append({'strategy':s,'direction':d,'baseline':b,'probability':p,'score':score,'model':m,'certified':cert,'threshold':th,'edge':edge,'regime_ok':rg,'phase_ok':phase_ok,'tradeable':trade})
