...
def setup(h4: list[dict], h1: list[dict], m15: list[dict], m5: list[dict],
    ...
    if formal:
        ...
        order_reason = "價格已進 OTE，但 15M／5M 或目標品質尚未全部確認；等待升級 L4"
    elif stage == 2:
        order_advice = "不建議掛單"
        order_zone = f"未開放（候選區 {min(entry):.2f} ～ {max(entry):.2f}）"
        preferred_entry = None
        order_reason = "目前只接近 OTE，尚未進入監控區"
    else:
        order_advice = "不建議掛單"
        order_zone = (f"未開放（候選區 {min(entry):.2f} ～ {max(entry):.2f}）"
    ...
    fib_id = (hashlib.sha256(
        ...
        f"{round(low, 2)}:{round(high, 2)}".encode()
    ).hexdigest()[:16] if fib_valid else None)
    fib_update_rule = (
        "目前沿用最近已確認的 1H BOS 推進波；出現更新的有效 BOS，且新終點右側 3 根 1H K 收完後自動換波"
        if fib_valid else
...
def discord_fields(result: dict, event_note: str = "") -> tuple[str, list[dict]]:
    ...
    fib_detail = (f"Fib `{'✅ 正式' if result.get('fib_valid') else '⏳ 無正式價位'}`｜{result.get('fib_reason', '—')}\n"
                  ...
                  f"換波規則：{result.get('fib_update_rule', '—')}")
...
async def notify(result: dict, previous: dict | None) -> None:
    ...
    if actionable_direction_change or protected_broken or became_invalid:
        ...
        await emit_alert(result, "INVALID", "⛔ ETH 結構破壞／Setup 失效｜目前不可下單", 0xE74C3C,
                         "；".join(reasons), old_id)
    if new_structure and not (fib_changed or actionable_direction_change or protected_broken or became_invalid):
...
fib.innerHTML=rows({'狀態':a.fib_valid?'正式有效（每次掃描重算）':'尚未建立','原因':a.fib_reason||'—','換波條件':a.fib_update_rule||'—','.618':a.fib?.['.618']??'—','.705':a.fib?.['.705']??'—','.786':a.fib?.['.786']??'—','OTE':a.ote?.length===2?a.ote.join(' ～ '):'—'});
