...
def setup(h4: list[dict], h1: list[dict], m15: list[dict], m5: list[dict],
    ...
    rr = [round(abs(x-entry_mid)/risk, 2) for x in targets] if risk else []
    ...
    if fib_valid and (not rr or max(rr) < 1.5):
...
plan.innerHTML=rows({'Setup 狀態':a.invalid?'已失效':a.zone_status||'—','進場區':a.fib_valid&&a.entry?.length===2?a.entry.join(' ～ '):'尚未提供','結構止損':a.stop||'—','流動性目標':tp||'等待','正式訊號':a.formal?'是':'否'});
