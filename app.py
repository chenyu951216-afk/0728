...
def setup(h4: list[dict], h1: list[dict], m15: list[dict], m5: list[dict],
    ...
    trend_reason = (f"自適應方向分數：多 {bull_votes:.2f}／空 {bear_votes:.2f}；"
    ...
    if fib_valid:
        entry = ([max(active_gap["low"], ote_low), min(active_gap["high"], ote_high)]
                 if active_gap else [ote_low, ote_high])
    else:
        entry = [None, None]
    if direction == "LONG" and fib_valid:
    ...
    elif fib_provisional:
        missing.append(
            "同方向推進仍在延伸：保留 1H 核心 OTE，另顯示 15M 戰術觀察區，等待端點正式確認"
            if adaptive_source == "15M 延伸子波" else
            "15M 戰術子波已失效：保留 1H 核心 OTE，並以 1H 延伸區繼續監控"
        )
    ...
    if fib_provisional:
        stage = min(stage, 3)
    formal = stage == 4 and not invalid and not fib_provisional
    public_stop = round(stop, 2) if formal else 0
    public_targets = targets if formal else []
    public_target_rr = rr if formal else []
    public_target_details = target_plan["details"] if formal else []
    if invalid:
        trade_status = "目前不可下單｜結構已失效，等待新 Setup"
    elif not fib_valid:
        trade_status = "目前不可下單｜尚未確認有效 BOS 推進波與正式 Fibonacci"
    elif fib_provisional:
        trade_status = "目前不可下單｜市場同方向延伸中；1H 核心 OTE 保留，自適應區僅供戰術觀察"
    ...
    if formal:
        order_advice = "可以評估限價掛單（不是市價追單）"
        order_zone = f"{min(entry):.2f} ～ {max(entry):.2f}"
        preferred_entry = round(fib[".705"], 2) if fib.get(".705") is not None else round(entry_mid, 2)
        order_reason = "L4：正式 Fib、OTE、15M MSS、5M 觸發與最低分數均已由收 K 確認"
    elif stage == 3:
        order_advice = "不建議掛單"
        order_zone = f"未開放（監控區 {min(entry):.2f} ～ {max(entry):.2f}）"
        preferred_entry = None
        order_reason = ("同方向延伸尚未確認；1H 核心區繼續監控，戰術區只作額外觀察"
                        if fib_provisional else
                        "價格已進 OTE，但 15M／5M 或目標品質尚未全部確認；等待升級 L4")
    elif stage == 2:
        order_advice = "不建議掛單"
        order_zone = f"未開放（候選區 {min(entry):.2f} ～ {max(entry):.2f}）"
        preferred_entry = None
        order_reason = ("同方向延伸尚未確認；1H 核心區仍保留，自適應區只作額外觀察，暫不掛單"
                        if fib_provisional else "目前只接近 OTE，尚未進入監控區")
    else:
        order_advice = "不建議掛單"
        order_zone = (f"未開放（候選區 {min(entry):.2f} ～ {max(entry):.2f}）"
                      if fib_valid else "尚未建立")
        preferred_entry = None
        order_reason = ("L1 僅為方向／結構觀察；" + ("、".join(missing[:3]) if missing else "正式條件尚未成立"))
    cancel_conditions = (
        "1H 主方向反轉、Fib 換波、1H 收盤破壞推進起點、價格超深穿越 OTE，或 L4 條件退回時取消／不得沿用"
    )
    ...
    fib_id = (hashlib.sha256(
        f"{direction}:{impulse_meta.get('origin_time')}:{impulse_meta.get('endpoint_time')}:"
        f"{round(confirmed_low, 2)}:{round(confirmed_high, 2)}".encode()
    ).hexdigest()[:16] if fib_valid else None)
    fib_update_rule = (
    ...
    return {
        ...
        "core_ote": ([round(confirmed_ote_low, 2), round(confirmed_ote_high, 2)]
                     if fib_valid else []),
        "tactical_ote": ([round(ote_low, 2), round(ote_high, 2)]
                         if fib_provisional and adaptive_source == "15M 延伸子波" else []),
...
def discord_fields(result: dict, event_note: str = "") -> tuple[str, list[dict]]:
    ...
    tps = ("L4 前不公布正式止損／止盈，不得沿用候選計算值"
           if not result.get("formal") else
    ...
    fib_state = ("⚡ 雙軌監控（1H 核心＋自適應觀察）" if result.get("fib_provisional") else
                 "✅ 正式" if result.get("fib_valid") else "⏳ 無正式價位")
    confirmed_ote = result.get("confirmed_ote", [])
    confirmed_note = (f"\n1H 核心 OTE `{n(confirmed_ote[0])}–{n(confirmed_ote[1])}` 繼續保留，直到 1H 結構失效或正式換波"
                      if result.get("fib_provisional") and len(confirmed_ote) == 2 else "")
    adaptive_note = (f"\n自適應來源 `{result.get('adaptive_source')}`｜延伸極值 "
                     f"`{n(result.get('extension_price'))}`｜子波起點 `{n(result.get('adaptive_origin'))}`"
                     if result.get("fib_provisional") else "")
    fib_detail = (f"Fib `{fib_state}`｜{result.get('fib_reason', '—')}\n"
                  ...
                  f"換波規則：{result.get('fib_update_rule', '—')}{confirmed_note}{adaptive_note}")
    ...
    plan_state = ("尚未建立（沒有正式 Fib，不應產生或沿用止損／止盈）"
                  if not result.get("fib_valid") else
                  "目前僅為候選觀察區；正式進場、止損與止盈尚未開放"
                  if not result.get("formal") else
                  f"進場 `{n(result['entry'][0])}–{n(result['entry'][1])}`\n"
                  f"結構止損 `{n(result['stop']) if result['stop'] else '—'}`")
    preferred = n(result.get("preferred_entry")) if result.get("preferred_entry") is not None else "—"
    quality_label = "正式目標品質" if result.get("formal") else "候選目標品質（不可據此掛單）"
    plan = (f"**掛單判定：{result.get('order_advice', '不建議掛單')}**\n"
            f"允許區間 `{result.get('order_zone', '尚未建立')}`｜優先參考價 `{preferred}`\n"
            f"原因：{result.get('order_reason', '—')}\n"
            f"{plan_state}\n{tps}\n"
            f"{quality_label} `{result.get('weighted_rr', 0):.2f}R`｜"
            f"{'✅ 合格' if result.get('target_quality_ok') else '❌ 不合格'}\n"
            f"取消條件：{result.get('cancel_conditions', '—')}")
    ...
    fields = [
        ...
        {"name": "🎯 參考交易計畫", "value": plan[:1024], "inline": False},
...
async def notify(result: dict, previous: dict | None) -> None:
    ...
    adaptive_started = bool(previous and result.get("fib_provisional") and
                            not previous.get("fib_provisional"))
    subwave_invalidated = bool(
        previous and previous.get("adaptive_source") == "15M 延伸子波" and
        str(result.get("adaptive_source", "")).startswith("1H 原推進波延伸")
    )
    ...
    if actionable_direction_change or protected_broken or became_invalid:
        reasons = []
        if actionable_direction_change:
            reasons.append(f"1H 主方向由 {previous['direction']} 轉為 {result['direction']}")
        if protected_broken:
    ...
    if fib_changed:
        ...
        old_ote = result.get("confirmed_ote", [])
        old_text = f"{n(old_ote[0])}–{n(old_ote[1])}" if len(old_ote) == 2 else "尚未建立"
        new_ote = result.get("ote", [])
        new_text = f"{n(new_ote[0])}–{n(new_ote[1])}" if len(new_ote) == 2 else "尚未建立"
        await emit_alert(
            ...
            0xF39C12,
            f"1H 核心 OTE `{old_text}` 繼續保留；另外新增自適應戰術觀察區 `{new_text}`。"
            "這不是反向換波，也不會取代核心區；等待新端點確認後才建立正式計畫。",
            result.get("fib_id") or result["setup_id"],
        )
    if subwave_invalidated and not fib_changed:
        ...
        new_ote = result.get("ote", [])
        new_text = f"{n(new_ote[0])}–{n(new_ote[1])}" if len(new_ote) == 2 else "尚未建立"
        old_origin = previous.get("adaptive_origin")
        await emit_alert(
            ...
            f"{result.get('fib_id') or result['setup_id']}:{n(old_origin)}",
        )
...
async def scan() -> None:
    ...
    result = setup(h4, h1, m15, m5, spot, ticker, spot_ticker, raw[6], previous)
...
fib.innerHTML=rows({'狀態':a.fib_provisional?'雙軌監控（核心區＋戰術區）':a.fib_valid?'正式有效（每次掃描重算）':'尚未建立','原因':a.fib_reason||'—','換波條件':a.fib_update_rule||'—','自適應來源':a.adaptive_source||'—','延伸極值 / 子波起點':a.fib_provisional?`${a.extension_price} / ${a.adaptive_origin}`:'—','1H 核心 OTE（持續保留）':a.core_ote?.length===2?a.core_ote.join(' ～ '):'—','15M 戰術觀察區（可失效）':a.tactical_ote?.length===2?a.tactical_ote.join(' ～ '):'—','目前優先監控區':a.ote?.length===2?a.ote.join(' ～ '):'—','.618':a.fib?.['.618']??'—','.705':a.fib?.['.705']??'—','.786':a.fib?.['.786']??'—'});
...
plan.innerHTML=rows({'目前是否建議掛單':a.order_advice||'不建議掛單','掛單允許區間':a.order_zone||'尚未建立','優先參考掛單價':a.preferred_entry??'—','判定原因':a.order_reason||'—','Setup 狀態':a.invalid?'已失效':a.zone_status||'—','候選觀察區':a.fib_valid&&a.entry?.length===2?a.entry.join(' ～ '):'尚未建立','正式進場區':a.formal&&a.entry?.length===2?a.entry.join(' ～ '):'尚未開放（L4 才提供）','正式結構止損':a.formal&&a.stop?a.stop:'尚未開放（不得沿用候選值）','正式 ICT 止盈':a.formal?(tp||'無合格目標'):'尚未開放（不得沿用候選值）','候選目標品質':`${a.weighted_rr??0}R / ${a.target_quality_ok?'合格':'不合格'}`,'目標判定':a.target_reason||'—','取消／失效條件':a.cancel_conditions||'—','正式訊號':a.formal?'是':'否'});
