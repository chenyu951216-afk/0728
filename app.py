...
def db() -> sqlite3.Connection:
    ...
    con.execute("CREATE TABLE IF NOT EXISTS snapshots(id INTEGER PRIMARY KEY,ts INTEGER NOT NULL,payload TEXT NOT NULL)")
    ...
    con.execute("""CREATE TABLE IF NOT EXISTS signals(
        ...
        post_mfe_r REAL NOT NULL DEFAULT 0,post_mae_r REAL NOT NULL DEFAULT 0,review_label TEXT)""")
...
def set_state(key: str, value: Any) -> None:
    con = db(); con.execute("""INSERT INTO system_state(key,value,updated_at) VALUES(?,?,?)
        ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at""",
        (key,json.dumps(value,ensure_ascii=False),int(time.time()))); con.commit(); con.close()


def insert_bars(source: str, asset: str, tf: str, rows: list[dict[str, Any]]) -> int:
    if not rows: return 0
    con = db(); before = con.total_changes
    con.executemany("INSERT OR IGNORE INTO market_bars(source,asset,tf,ts,o,h,l,c,v,qv) VALUES(?,?,?,?,?,?,?,?,?,?)",
        [(source,asset,tf,int(x["ts"]),float(x["o"]),float(x["h"]),float(x["l"]),float(x["c"]),float(x.get("v",0)),float(x.get("qv",0))) for x in rows])
    added = con.total_changes-before; con.commit(); con.close(); return added
...
def _raw_derivatives(bundle: dict[str, Any]) -> dict[str, float]:
    raw,ticker=bundle.get("derivatives_raw",{}),bundle.get("ticker",{}); funding_values=[]; oi_values=[]
    ...
    bid_qty=sum(abs(float(x.get("s",0) if isinstance(x,dict) else x[1])) for x in bids[:30]); ask_qty=sum(abs(float(x.get("s",0) if isinstance(x,dict) else x[1])) for x in asks[:30]); imbalance=(bid_qty-ask_qty)/max(bid_qty+ask_qty,1e-9)
    perp=float(ticker.get("last") or 0); spot_rows=bundle.get("eth_spot_15m",[]); spot=float(spot_rows[-1]["c"]) if spot_rows else 0.0
    return {"funding":statistics.median(funding_values) if funding_values else 0.0,"oi_change":oi_change,"book_imbalance":imbalance,
            "spot_perp_basis_bps":(perp/spot-1)*10000 if perp and spot else 0.0,"source_agreement_bps":bundle.get("quality",{}).get("agreement_median_bps") or 999.0}
...
def _entry_for_strategy(strategy: str, direction: str, live: float, m15: list[dict[str, Any]]) -> float:
    ...
    if strategy=="TREND_PULLBACK": proposed=min(live-0.10*a,e20) if direction=="LONG" else max(live+0.10*a,e20)
...
def _notional_for_risk(entry: float, stop: float) -> dict[str, Any]:
    ...
    stop_pct=abs(entry-stop)/entry; return {"equity_usdt":equity,"risk_pct":RISK_PER_TRADE,"stop_pct":stop_pct,"notional_usdt":round(equity*RISK_PER_TRADE/max(stop_pct,1e-6),2)}
...
def create_signal(analysis: dict[str,Any], m15: list[dict[str,Any]]) -> dict[str,Any] | None:
    ...
    payload={"initial_plan":plan,"selection":selection,"regime":analysis["regime"],"features":analysis.get("features",{}),
             "data_quality":float((analysis.get("data_quality") or {}).get("score",0)),"created_from_snapshot":analysis.get("snapshot_ts"),"immutable":True,
             "management":{"hit_targets":[],"mfe_r":0.0,"mae_r":0.0,"trail_reason":None}}
    con=db(); con.execute("""INSERT INTO signals(signal_id,created_at,updated_at,status,strategy,direction,regime,phase,probability,entry,initial_stop,current_stop,targets,payload)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(signal_id,int(time.time()),int(time.time()),"PLANNED",selection["strategy"],selection["direction"],analysis["regime"]["regime"],analysis["regime"]["phase"],selection["probability"],plan["entry"],plan["stop"],plan["stop"],json.dumps(plan["targets"]),json.dumps(payload,ensure_ascii=False))); con.commit(); con.close(); return latest_signal()


def _close_signal(row: dict[str,Any], price: float, reason: str, ts: int) -> None:
    entry,stop0=float(row["entry"]),float(row["initial_stop"]); sign=1 if row["direction"]=="LONG" else -1; risk=abs(entry-stop0) or 1e-9; realized_r=(price-entry)*sign/risk
    payload=row["payload"] if isinstance(row["payload"],dict) else json.loads(row["payload"]); payload.setdefault("management",{})["closed_reason"]=reason
...
def update_signal_with_bar(bar: dict[str,Any]) -> dict[str,Any] | None:
    row=latest_signal()
    if not row: return None
    ts=int(bar["ts"]); entry,stop0,current_stop=float(row["entry"]),float(row["initial_stop"]),float(row["current_stop"]); direction=row["direction"]; sign=1 if direction=="LONG" else -1; low,high=float(bar["l"]),float(bar["h"]); touched=low<=entry<=high; payload,targets=row["payload"],row["targets"]
    ...
    risk=abs(entry-stop0) or 1e-9; favorable=(high-entry)/risk if direction=="LONG" else (entry-low)/risk; adverse=(entry-low)/risk if direction=="LONG" else (high-entry)/risk
    mgmt=payload.setdefault("management",{}); mgmt["mfe_r"]=max(float(mgmt.get("mfe_r",0)),favorable); mgmt["mae_r"]=max(float(mgmt.get("mae_r",0)),adverse); hit_targets=list(mgmt.get("hit_targets",[]))
    if (low<=current_stop if direction=="LONG" else high>=current_stop): _close_signal(row,current_stop,"STOP_OR_TRAIL",ts); return None
...
def post_exit_review(bar: dict[str,Any]) -> None:
    ...
    for raw in rows:
        ...
        favorable=(float(bar["h"])-exit_price)/risk if sign>0 else (exit_price-float(bar["l"]))/risk; adverse=(exit_price-float(bar["l"]))/risk if sign>0 else (float(bar["h"])-exit_price)/risk
...
def _earliest(asset: str, tf: str, source: str | None = None) -> int | None:
    con=db()
    if source: row=con.execute("SELECT MIN(ts) FROM market_bars WHERE source=? AND asset=? AND tf=?",(source,asset,tf)).fetchone()
    else: row=con.execute("SELECT MIN(ts) FROM market_bars WHERE asset=? AND tf=?",(asset,tf)).fetchone()
    con.close(); return int(row[0]) if row and row[0] is not None else None
...
def _best_source(asset: str, tf: str) -> str | None:
    ...
    if full:
        full.sort(key=lambda r:(r[0]!="gate",-int(r[1]))); return str(full[0][0])
    return str(rows[0][0])
...
def generate_learning_samples(batch: int = 450) -> int:
    ...
    ts15,ts1,ts4,tsd,tsb=[x["ts"] for x in m15],[x["ts"] for x in h1],[x["ts"] for x in h4],[x["ts"] for x in d1],[x["ts"] for x in btc]; last_ts=int(get_state("last_learning_sample_ts",START_TS) or START_TS); start_i=max(100,bisect.bisect_right(ts15,last_ts)); con=db(); store=ModelStore(con); learner=Learner(store); created=processed=0; newest=last_ts
    ...
    for i in range(start_i,len(m15)-25):
        ...
        for strategy,meta in candidates.items():
            ...
            success,pnl_r,mfe_r,mae_r=learner.outcome(m15,i,meta["direction"],24); store.add_sample({"ts":ts,"strategy":strategy,"direction":meta["direction"],"regime":regime["regime"],"phase":regime["phase"],"features":features,"success":success,"pnl_r":pnl_r,"mfe_r":mfe_r,"mae_r":mae_r,"source_quality":82.0 if src15=="gate" else 74.0}); created+=1
...
def ingest_completed_live_samples() -> int:
    """Reviewed live/paper trades become samples, never direct model mutations."""
    ...
    for raw in rows:
        ...
        mgmt=payload.get("management") or {}; realized=float(row["realized_r"] or 0)
        store.add_sample({"ts":int(row["created_at"]),"strategy":row["strategy"],"direction":row["direction"],"regime":row["regime"],"phase":row["phase"],"features":features,"success":int(realized>0.15),"pnl_r":realized,"mfe_r":max(float(mgmt.get("mfe_r",0)),float(row["post_mfe_r"] or 0)),"mae_r":max(float(mgmt.get("mae_r",0)),float(row["post_mae_r"] or 0)),"source_quality":max(55.0,float(payload.get("data_quality",75.0)))})
        payload["learning_ingested"]=int(time.time()); con.execute("UPDATE signals SET payload=? WHERE signal_id=?",(json.dumps(payload,ensure_ascii=False),row["signal_id"])); added+=1
...
async def learning_tick() -> None:
    ...
    con.close(); state["learning"]={"progress":progress,"backfill":backfill_result,"live_samples_added":live_added,"sample_counts":counts,"champions":champions,"learning_order":["1D/4H","1H/30M","15M/5M","live/post-exit"]}
...
app=FastAPI(title="ETH Adaptive Short-Term Engine",version="4.1.0",lifespan=lifespan)
...
def backtest(strategy: str | None=None, limit: int=Query(5000,ge=100,le=20000)) -> dict[str,Any]:
    ...
    for name in strategies:
        rows=con.execute("SELECT pnl_r,success,regime FROM learning_samples WHERE strategy=? ORDER BY ts DESC LIMIT ?",(name,limit)).fetchall(); pnls=[float(x[0]) for x in rows]; wins=[int(x[1]) for x in rows]; gains=sum(max(x,0) for x in pnls); losses=sum(max(-x,0) for x in pnls); by_regime={}
        for row in rows: by_regime.setdefault(row[2],[]).append(float(row[0]))
        result[name]={"n":len(rows),"win_rate":sum(wins)/len(wins) if wins else 0,"expectancy_r":statistics.mean(pnls) if pnls else 0,"profit_factor":gains/max(losses,1e-9),"by_regime":{k:{"n":len(v),"ev_r":statistics.mean(v)} for k,v in by_regime.items()}}
    con.close(); return result
@app.get("/",response_class=HTMLResponse)
def index() -> str:
    return """<!doctype html><html lang='zh-Hant'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>ETH Adaptive Engine</title><style>body{font-family:system-ui;background:#0b1020;color:#e8edf7;margin:0;padding:24px}.card{background:#151d33;border:1px solid #2a3555;border-radius:16px;padding:18px;margin:12px 0}pre{white-space:pre-wrap}.ok{color:#67e8a5}.warn{color:#ffd166}</style></head><body><h1>ETH Adaptive Short-Term Engine v4.1</h1><div class='card'><b>Learning order</b><p>1D/4H → 1H/30M → 15M/5M → live + post-exit review</p></div><div id='app' class='card'>loading...</div><script>async function tick(){let x=await fetch('/api/status').then(r=>r.json());let a=x.analysis||{},l=x.learning||{},s=x.active_signal;document.getElementById('app').innerHTML=`<b class='${x.service==='OK'?'ok':'warn'}'>${x.service}</b><pre>Regime: ${JSON.stringify(a.regime||{},null,2)}\nSelection: ${JSON.stringify(a.selection||{},null,2)}\nActive: ${JSON.stringify(s||null,null,2)}\nLearning: ${JSON.stringify(l.progress||{},null,2)}</pre>`}tick();setInterval(tick,5000)</script></body></html>"""
