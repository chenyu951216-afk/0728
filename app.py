...
def setup(h4: list[dict], h1: list[dict], m15: list[dict], m5: list[dict],
    ...
    s4, s15, s5 = structure(h4), structure(m15), structure(m5)
    ...
    structure_id = hashlib.sha256(
        f"{s4['trend']}:{latest_high['t']}:{latest_high['p']}:{latest_low['t']}:{latest_low['p']}".encode()
    ).hexdigest()[:16]
    ...
    return {
        ...
        "setup_id": setup_id, "structure_id": structure_id, "trend_4h": s4["trend"],
        ...
        "invalid": invalid, "invalid_reason": invalid_reason, "formal": stage == 4 and not invalid,
...
def discord_fields(result: dict, event_note: str = "") -> tuple[str, list[dict]]:
    ...
    structure_text = (f"方向 **{result['direction']}**｜4H **{result['trend_4h']}**｜ADX `{result['adx_4h']}`\n"
    ...
    description = (f"**分數 {result['score']}/100｜階段 L{result['stage']}｜"
                   f"{'正式確認' if result['formal'] else '尚未正式確認'}**")
    if event_note:
        description += f"\n{event_note}"
    fields = [
...
async def notify(result: dict, previous: dict | None) -> None:
    ...
    stage_titles = {
        1: ("🧭 ETH 新結構／趨勢觀察 L1", 0x5B8FF9),
        2: ("🔔 ETH 接近 OTE L2", 0xF6BD16),
        3: ("🟠 ETH 進入 OTE 監控區 L3", 0xE8684A),
        4: ("✅ ETH 交易條件收 K 確認 L4", 0x5AD8A6),
    }
    ...
    if direction_changed or protected_broken or (result["invalid"] and not (previous or {}).get("invalid")):
        ...
        await emit_alert(result, "INVALID", "⛔ ETH 結構破壞／Setup 失效", 0xE74C3C,
                         "；".join(reasons), old_id)
    if new_structure:
        await emit_alert(result, "NEW_STRUCTURE", "🆕 ETH 已收 K 新結構建立", 0x3498DB,
    ...
    if result["stage"] > previous_stage or not previous:
        ...
        await emit_alert(result, f"L{result['stage']}", title, color)
    elif previous and previous.get("formal") and not result["formal"]:
        await emit_alert(result, "CONDITION_LOST", "⚠️ ETH 正式條件已退回", 0xF39C12,
...
direction.textContent=(a.direction||'—')+' · L'+(a.stage||0)+' · '+(a.zone_status||'—');
