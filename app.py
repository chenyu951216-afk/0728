...
def complete_target_plan(direction: str, entry_mid: float, stop: float,
    ...
    """保留 ICT 流動性目標；不足四階時補明確標示的 R 倍數風控目標。"""
    risk = abs(entry_mid-stop)
    if risk <= 0:
        return []
    ...
    result = []
    allocations = (25, 30, 30, 15)
    for index, minimum_rr in enumerate((1.0, 1.5, 2.0, 2.5)):
        match = next(
            (x for x in actual
             if f(x.get("rr")) >= minimum_rr and
        ...
        result.append({
            ...
            "type": target_type, "allocation": allocations[index],
        })
    return result
...
def make_trade_plan(kind: str, direction: str, low: float, high: float,
                    ...
                    impulse_meta: dict, previous_plan: dict | None = None) -> dict:
    """建立可持續的計畫生命週期；價位固定、觸區與最高階段會跨掃描保留。"""
    fib, raw_zone = fib_zone(direction, low, high)
    zone = [round(raw_zone[0], 2), round(raw_zone[1], 2)]
    ...
    entry_mid = sum(raw_zone)/2
    liquidity_targets = ict_targets(direction, entry_mid, stop, live_price, h4, h1, m15)
    targets = prior.get("targets") or complete_target_plan(
        direction, entry_mid, stop, liquidity_targets
    )
    stop = f(prior.get("stop"), stop) if prior.get("stop") is not None else stop
    bar_touched = f(last_bar.get("h")) >= raw_zone[0] and f(last_bar.get("l")) <= raw_zone[1]
    in_zone = raw_zone[0] <= live_price <= raw_zone[1]
    zone_reached = bool(prior.get("zone_reached") or in_zone or bar_touched)
    distance = .35*(atr(h1) if kind == "MAIN" else atr(m15))
    if direction == "LONG":
        near_zone = raw_zone[1] < live_price <= raw_zone[1]+distance
        adverse = live_price < raw_zone[0]
        favorable_departure = zone_reached and live_price > raw_zone[1]
        confirmation_window_open = (
            in_zone or
            (zone_reached and raw_zone[1] < live_price <= raw_zone[1] +
             (atr(h1) if kind == "MAIN" else atr(m15)))
        )
        invalid = closed_1h < low if kind == "MAIN" else m15[-1]["c"] < low
    else:
        near_zone = raw_zone[0]-distance <= live_price < raw_zone[0]
        adverse = live_price > raw_zone[1]
        favorable_departure = zone_reached and live_price < raw_zone[0]
        confirmation_window_open = (
            in_zone or
            (zone_reached and raw_zone[0] -
             (atr(h1) if kind == "MAIN" else atr(m15)) <= live_price < raw_zone[0])
        )
        invalid = closed_1h > high if kind == "MAIN" else m15[-1]["c"] > high
    ready_now = bool(not invalid and zone_reached and confirmation_window_open and
                     score >= MIN_SCORE and mss and trigger_5m)
    ...
    if invalid:
        ...
        status = "READY"
        action = (f"可評估在 {zone[0]:.2f}～{zone[1]:.2f} 限價掛單；"
                  f"止損 {stop:.2f}，依序管理 TP1～TP4，不要市價追單")
    elif stage >= 4:
        status = "COOLDOWN"
        action = "L4 曾確認但目前不在可掛單窗口；只管理既有掛單／持倉，不新增追價單"
    elif zone_reached:
        status = "TRIGGER"
        action = "已到計畫區；保持原方向，等待 15M MSS 與 5M 觸發收 K，不做反方向"
    elif near_zone:
        status = "APPROACH"
        action = "接近計畫區；準備觀察，不提前掛單、不追價"
    else:
        status = "WAIT"
        action = "保留計畫價位，等待價格接近掛單區；現在不下單"
    if invalid:
        zone_status = "結構已失效"
    elif in_zone:
        zone_status = "目前位於計畫區"
    ...
    missing = []
    if not zone_reached:
        missing.append("價格尚未觸及計畫區")
    if not mss:
        missing.append("15M MSS 尚未確認")
    if not trigger_5m:
        missing.append("5M 觸發尚未確認")
    if score < MIN_SCORE:
    ...
    return {
        ...
        "zone": zone, "entry": zone, "preferred_entry": round(fib[".705"], 2),
        "stop": round(stop, 2), "targets": targets,
        "target_quality_ok": liquidity_targets["quality_ok"],
        "target_reason": liquidity_targets["reason"],
        "weighted_rr": liquidity_targets["weighted_rr"],
        "zone_reached": zone_reached,
        "zone_reached_at": (prior.get("zone_reached_at") or
...
def setup(h4: list[dict], h1: list[dict], m15: list[dict], m5: list[dict],
    ...
    if fib_valid and direction in ("LONG", "SHORT"):
        main_plan = make_trade_plan(
            ...
            m5[-1], score, mss, trigger_5m, spread, h4, h1, m15,
            impulse_meta, old_main_plan,
        )
...
def discord_fields(result: dict, event_note: str = "") -> tuple[str, list[dict]]:
    def plan_text(plan: dict | None) -> str:
        ...
        return (
            ...
            f"{plan.get('status', '—')}**\n"
            f"掛單區 `{zone_text}`｜優先 `{n(plan.get('preferred_entry'))}`\n"
...
async def notify(result: dict, previous: dict | None) -> None:
    ...
    if protected_broken and not (actionable_direction_change or became_invalid):
        await emit_alert(
            ...
            "只重置小週期確認，不反轉、不刪除主計畫，等待新的 15M／5M 收 K。",
            result.get("setup_id"),
        )
...
app = FastAPI(title="ETH Trading Secretary", version="3.0.0", lifespan=lifespan)
