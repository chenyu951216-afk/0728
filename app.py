...
def setup(h4: list[dict], h1: list[dict], m15: list[dict], m5: list[dict],
    ...
    if fib_valid:
        ...
        if fib_provisional:
            ...
            if direction == "LONG":
                ...
                if extension_price-candidate_origin >= .75*atr(m15):
    ...
    if not fib_valid: missing.append(f"未建立正式 Fibonacci：{impulse_meta['reason']}")
        ...
        missing.append("同方向推進仍在延伸：舊 OTE 已暫停，顯示自適應觀察區，等待端點正式確認")
    ...
    if invalid:
        ...
        trade_status = "目前不可下單｜市場同方向延伸中，舊 OTE 已暫停；目前為自適應觀察區"
    ...
    if formal:
        ...
        order_reason = ("同方向延伸尚未確認，舊計畫已暫停；等待新端點確認"
        ...
        preferred_entry = None
        order_reason = ("同方向延伸尚未確認，候選區會隨市場自適應；暫不掛單"
    ...
    fib_update_rule = (
        "同方向推進正在延伸：正式舊 OTE 已暫停，先顯示自適應觀察區；端點確認後自動升格"
        if fib_provisional else
        "目前沿用最近已確認的 1H BOS 推進波；同向延伸會即時暫停舊區，端點確認後正式更新"
        if fib_valid else
...
def discord_fields(result: dict, event_note: str = "") -> tuple[str, list[dict]]:
    ...
    fib_state = ("⚡ 自適應觀察（舊正式區已暫停）" if result.get("fib_provisional") else
                 "✅ 正式" if result.get("fib_valid") else "⏳ 無正式價位")
    confirmed_ote = result.get("confirmed_ote", [])
    confirmed_note = (f"\n前次正式 OTE `{n(confirmed_ote[0])}–{n(confirmed_ote[1])}` 已暫停沿用"
...
async def notify(result: dict, previous: dict | None) -> None:
    ...
    if actionable_direction_change or protected_broken or became_invalid:
        ...
        await emit_alert(result, "INVALID", "⛔ ETH 結構破壞／Setup 失效｜目前不可下單", 0xE74C3C,
                         "；".join(reasons), old_id)
    if new_structure and not (fib_changed or adaptive_started or actionable_direction_change or
                              protected_broken or became_invalid):
    ...
    if fib_changed:
        ...
        new_text = f"{n(new_ote[0])}–{n(new_ote[1])}" if len(new_ote) == 2 else "尚未建立"
        await emit_alert(
            result, "ADAPTIVE_EXTENSION", "⚡ ETH 同向推進延伸｜舊掛單計畫立即暫停",
            0xF39C12,
            f"前次正式 OTE `{old_text}` 暫停沿用；目前自適應觀察區 `{new_text}`。"
            "這不是反向換波，等待新端點確認後才建立正式計畫。",
...
fib.innerHTML=rows({'狀態':a.fib_provisional?'自適應觀察（舊正式區已暫停）':a.fib_valid?'正式有效（每次掃描重算）':'尚未建立','原因':a.fib_reason||'—','換波條件':a.fib_update_rule||'—','自適應來源':a.adaptive_source||'—','延伸極值 / 子波起點':a.fib_provisional?`${a.extension_price} / ${a.adaptive_origin}`:'—','前次正式 OTE':a.confirmed_ote?.length===2?a.confirmed_ote.join(' ～ '):'—','目前有效／觀察 OTE':a.ote?.length===2?a.ote.join(' ～ '):'—','.618':a.fib?.['.618']??'—','.705':a.fib?.['.705']??'—','.786':a.fib?.['.786']??'—'});
...
plan.innerHTML=rows({'目前是否建議掛單':a.order_advice||'不建議掛單','掛單允許區間':a.order_zone||'尚未建立','優先參考掛單價':a.preferred_entry??'—','判定原因':a.order_reason||'—','Setup 狀態':a.invalid?'已失效':a.zone_status||'—','進場區':a.fib_valid&&a.entry?.length===2?a.entry.join(' ～ '):'尚未建立（不可沿用舊值）','結構止損':a.fib_valid&&a.stop?a.stop:'尚未建立（不可沿用舊值）','ICT 流動性目標':a.fib_valid?(tp||'無合格目標'):'尚未建立（不可沿用舊值）','加權報酬':`${a.weighted_rr??0}R / ${a.target_quality_ok?'合格':'不合格'}`,'目標判定':a.target_reason||'—','取消／失效條件':a.cancel_conditions||'—','正式訊號':a.formal?'是':'否'});
