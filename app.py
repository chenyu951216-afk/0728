...
from adaptive_engine import (
    Learner, ModelStore, STRATEGIES, atr, bootstrap_progress, build_features,
    choose_strategy, detect_regime, ema, risk_plan,
)
...
hub = MarketDataHub()
state: dict[str, Any] = {
    "service": "BOOTING", "updated_at": None, "error": None, "scan_count": 0,
    "analysis": {}, "learning": {}, "active_signal": None, "last_training": None,
...
def db() -> sqlite3.Connection:
    ...
    con.execute("""CREATE TABLE IF NOT EXISTS snapshots(
        id INTEGER PRIMARY KEY, ts INTEGER NOT NULL, payload TEXT NOT NULL)""")
    ...
    con.execute("""CREATE TABLE IF NOT EXISTS source_ledger(
        ts INTEGER NOT NULL, source TEXT NOT NULL, feature_group TEXT NOT NULL,
        ok INTEGER NOT NULL, quality REAL NOT NULL, detail TEXT NOT NULL,
        PRIMARY KEY(ts,source,feature_group))""")
    con.execute("""CREATE TABLE IF NOT EXISTS signals(
        signal_id TEXT PRIMARY KEY, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL,
        status TEXT NOT NULL, strategy TEXT NOT NULL, direction TEXT NOT NULL,
        regime TEXT NOT NULL, phase TEXT NOT NULL, probability REAL NOT NULL,
        entry REAL NOT NULL, initial_stop REAL NOT NULL, current_stop REAL NOT NULL,
        targets TEXT NOT NULL, payload TEXT NOT NULL, filled_at INTEGER, exit_ts INTEGER,
        exit_price REAL, exit_reason TEXT, realized_r REAL, review_until INTEGER,
        post_mfe_r REAL NOT NULL DEFAULT 0, post_mae_r REAL NOT NULL DEFAULT 0,
        review_label TEXT)""")
    con.execute("CREATE INDEX IF NOT EXISTS ix_signals_status ON signals(status,created_at)")
    con.execute("""CREATE TABLE IF NOT EXISTS alerts(
        id INTEGER PRIMARY KEY, ts INTEGER NOT NULL, signal_id TEXT, kind TEXT NOT NULL, message TEXT NOT NULL)""")
    ModelStore(con)
    con.commit()
    return con
...
def insert_bars(source: str, asset: str, tf: str, rows: list[dict[str, Any]]) -> int:
    ...
    con.executemany("""INSERT OR IGNORE INTO market_bars(source,asset,tf,ts,o,h,l,c,v,qv)
                       VALUES(?,?,?,?,?,?,?,?,?,?)""",
                    [(source, asset, tf, int(x["ts"]), float(x["o"]), float(x["h"]), float(x["l"]),
                      float(x["c"]), float(x.get("v", 0)), float(x.get("qv", 0))) for x in rows])
...
def load_bars(asset: str, tf: str, source: str = "gate", limit: int | None = None) -> list[dict[str, Any]]:
    con = db()
    sql = "SELECT ts,o,h,l,c,v,qv FROM market_bars WHERE source=? AND asset=? AND tf=? ORDER BY ts"
    params: list[Any] = [source, asset, tf]
    if limit:
        sql = "SELECT * FROM (" + sql + " DESC LIMIT ?) ORDER BY ts"
        params.append(limit)
    rows = con.execute(sql, params).fetchall(); con.close()
    return [{"ts": r[0], "o": r[1], "h": r[2], "l": r[3], "c": r[4], "v": r[5], "qv": r[6]} for r in rows]


def upsert_live_gate(bundle: dict[str, Any]) -> None:
    for key, tf in (("eth_1d", "1d"), ("eth_4h", "4h"), ("eth_1h", "1h"), ("eth_30m", "30m"),
                    ("eth_15m", "15m"), ("eth_5m", "5m"), ("btc_1h", "1h")):
        insert_bars("gate", "BTC" if key == "btc_1h" else "ETH", tf, bundle.get(key, []))
    for source, rows in bundle.get("validators", {}).items():
        insert_bars(source, "ETH", "15m", rows)


def _raw_derivatives(bundle: dict[str, Any]) -> dict[str, float]:
    raw, ticker = bundle.get("derivatives_raw", {}), bundle.get("ticker", {})
    funding_values, oi_values = [], []
    ...
    bids, asks = bundle.get("orderbook", {}).get("bids", []), bundle.get("orderbook", {}).get("asks", [])
    bid_qty = sum(abs(float(x.get("s", 0) if isinstance(x, dict) else x[1])) for x in bids[:30])
    ask_qty = sum(abs(float(x.get("s", 0) if isinstance(x, dict) else x[1])) for x in asks[:30])
    imbalance = (bid_qty - ask_qty) / max(bid_qty + ask_qty, 1e-9)
    perp = float(ticker.get("last") or 0); spot_rows = bundle.get("eth_spot_15m", []); spot = float(spot_rows[-1]["c"]) if spot_rows else 0.0
    return {"funding": statistics.median(funding_values) if funding_values else 0.0, "oi_change": oi_change,
            "book_imbalance": imbalance, "spot_perp_basis_bps": (perp / spot - 1) * 10000 if perp and spot else 0.0,
            "source_agreement_bps": bundle.get("quality", {}).get("agreement_median_bps") or 999.0}
...
def _entry_for_strategy(strategy: str, direction: str, live: float, m15: list[dict[str, Any]]) -> float:
    ...
    if strategy == "TREND_PULLBACK":
        proposed = min(live - 0.10 * a, e20) if direction == "LONG" else max(live + 0.10 * a, e20)
...
def _notional_for_risk(entry: float, stop: float) -> dict[str, Any]:
    ...
    if equity <= 0 or entry <= 0 or stop <= 0:
        return {"equity_usdt": equity, "risk_pct": RISK_PER_TRADE, "notional_usdt": None, "reason": "set account equity first"}
    stop_pct = abs(entry - stop) / entry
    return {"equity_usdt": equity, "risk_pct": RISK_PER_TRADE, "stop_pct": stop_pct,
            "notional_usdt": round(equity * RISK_PER_TRADE / max(stop_pct, 1e-6), 2)}


def latest_signal(statuses: tuple[str, ...] = ("PLANNED", "OPEN")) -> dict[str, Any] | None:
    con = db(); placeholders = ",".join("?" for _ in statuses)
    row = con.execute(f"SELECT * FROM signals WHERE status IN ({placeholders}) ORDER BY created_at DESC LIMIT 1", statuses).fetchone(); con.close()
...
def create_signal(analysis: dict[str, Any], m15: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    live = analysis["price"]; entry = _entry_for_strategy(selection["strategy"], selection["direction"], live, m15)
    con = db(); store = ModelStore(con); plan = risk_plan(store, selection["strategy"], analysis["regime"]["regime"], selection["direction"], entry, m15); con.close()
    signal_id = f"{int(time.time())}-{selection['strategy'][:4]}-{selection['direction'][0]}"
    payload = {"initial_plan": plan, "selection": selection, "regime": analysis["regime"], "created_from_snapshot": analysis.get("snapshot_ts"),
               "immutable": True, "management": {"hit_targets": [], "mfe_r": 0.0, "mae_r": 0.0, "trail_reason": None}}
    con = db()
    con.execute("""INSERT INTO signals(signal_id,created_at,updated_at,status,strategy,direction,regime,phase,
                   probability,entry,initial_stop,current_stop,targets,payload) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (signal_id, int(time.time()), int(time.time()), "PLANNED", selection["strategy"], selection["direction"],
                 analysis["regime"]["regime"], analysis["regime"]["phase"], selection["probability"], plan["entry"], plan["stop"], plan["stop"],
                 json.dumps(plan["targets"]), json.dumps(payload, ensure_ascii=False)))
    con.commit(); con.close(); return latest_signal()


def _close_signal(row: dict[str, Any], price: float, reason: str, ts: int) -> None:
    entry, stop0 = float(row["entry"]), float(row["initial_stop"]); sign = 1 if row["direction"] == "LONG" else -1; risk = abs(entry - stop0) or 1e-9
    realized_r = (price - entry) * sign / risk; payload = row["payload"] if isinstance(row["payload"], dict) else json.loads(row["payload"])
    payload.setdefault("management", {})["closed_reason"] = reason
    con = db(); con.execute("""UPDATE signals SET status='CLOSED',updated_at=?,exit_ts=?,exit_price=?,exit_reason=?,realized_r=?,review_until=?,payload=? WHERE signal_id=?""",
                            (ts, ts, price, reason, realized_r, ts + POST_EXIT_BARS * 900, json.dumps(payload, ensure_ascii=False), row["signal_id"])); con.commit(); con.close()


def update_signal_with_bar(bar: dict[str, Any]) -> dict[str, Any] | None:
    row = latest_signal()
    if not row: return None
    ts = int(bar["ts"]); entry, stop0, current_stop = float(row["entry"]), float(row["initial_stop"]), float(row["current_stop"])
    direction = row["direction"]; sign = 1 if direction == "LONG" else -1; low, high = float(bar["l"]), float(bar["h"])
    touched = low <= entry <= high; payload, targets = row["payload"], row["targets"]
    if row["status"] == "PLANNED":
        if not touched:
            ...
            return row
        con = db(); con.execute("UPDATE signals SET status='OPEN',filled_at=?,updated_at=? WHERE signal_id=?", (ts, ts, row["signal_id"])); con.commit(); con.close(); row["status"] = "OPEN"; row["filled_at"] = ts
    ...
    mgmt = payload.setdefault("management", {}); mgmt["mfe_r"] = max(float(mgmt.get("mfe_r", 0)), favorable); mgmt["mae_r"] = max(float(mgmt.get("mae_r", 0)), adverse)
    hit_targets = list(mgmt.get("hit_targets", []))
    if (low <= current_stop if direction == "LONG" else high >= current_stop):
        _close_signal(row, current_stop, "STOP_OR_TRAIL", ts); return None
...
def post_exit_review(bar: dict[str, Any]) -> None:
    ts = int(bar["ts"]); con = db()
    rows = con.execute("SELECT * FROM signals WHERE status='CLOSED' AND review_label IS NULL AND review_until>=?", (ts,)).fetchall()
    for raw in rows:
        row = dict(raw)
        if not row["exit_ts"] or ts <= row["exit_ts"]: continue
        entry, stop0 = float(row["entry"]), float(row["initial_stop"]); risk = abs(entry - stop0) or 1e-9; sign = 1 if row["direction"] == "LONG" else -1; exit_price = float(row["exit_price"])
        favorable = (float(bar["h"]) - exit_price) / risk if sign > 0 else (exit_price - float(bar["l"])) / risk
        adverse = (exit_price - float(bar["l"])) / risk if sign > 0 else (float(bar["h"]) - exit_price) / risk
        con.execute("UPDATE signals SET post_mfe_r=MAX(post_mfe_r,?),post_mae_r=MAX(post_mae_r,?),updated_at=? WHERE signal_id=?", (favorable, adverse, ts, row["signal_id"]))
    expired = con.execute("SELECT * FROM signals WHERE status='CLOSED' AND review_label IS NULL AND review_until<?", (ts,)).fetchall()
    for raw in expired:
        row = dict(raw); realized, post_mfe = float(row["realized_r"] or 0), float(row["post_mfe_r"] or 0)
...
async def send_discord(title: str, body: str, color: int = 0x5B8FF9) -> None:
    if not DISCORD_WEBHOOK_URL and not (DISCORD_BOT_TOKEN and DISCORD_CHANNEL_ID): return
    payload = {"embeds": [{"title": title, "description": body[:4000], "color": color, "timestamp": datetime.now(timezone.utc).isoformat(),
                           "footer": {"text": "ETH Adaptive Engine | research/paper only"}}]}
...
def _analysis_from_bundle(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    regime = detect_regime(d1, h4, h1); extras = _raw_derivatives(bundle); features = build_features(m15, h1, btc, regime, extras)
    con = db(); store = ModelStore(con); learner = Learner(store); selection = choose_strategy(store, learner, features, regime, bundle["quality"]["score"]); con.close()
    ticker = bundle["ticker"]; price = float(ticker.get("last") or m15[-1]["c"])
    return {"snapshot_ts": int(time.time()), "price": price, "regime": regime, "features": features, "selection": selection,
            "data_quality": bundle["quality"], "derivatives": extras, "trade_label": "LEARNED TRADE" if selection["tradeable"] else "WAIT / LEARNING",
            "rule": "strategy selection prioritizes certified learned champion models"}


async def scan() -> dict[str, Any]:
    bundle = await hub.live_bundle(); upsert_live_gate(bundle); analysis = _analysis_from_bundle(bundle); now = int(time.time())
    con = db(); con.execute("INSERT INTO snapshots(ts,payload) VALUES(?,?)", (now, json.dumps(analysis, ensure_ascii=False))); con.execute("DELETE FROM snapshots WHERE ts<?", (now - 120 * 86400,)); con.commit(); con.close()
    last_bar = bundle["eth_15m"][-1]; update_signal_with_bar(last_bar); post_exit_review(last_bar); before = latest_signal()
    if before is None:
        created = create_signal(analysis, bundle["eth_15m"])
        if created and created["created_at"] >= now - 5:
            await send_discord("ETH learned setup created", f"{created['direction']} | {created['strategy']} | regime {created['regime']}/{created['phase']}\nprobability {created['probability']:.1%} | entry limit {created['entry']} | stop {created['initial_stop']}\nTargets: " + ", ".join(str(x['price']) for x in created['targets']) + "\nNo market chase; initial plan is immutable.", 0x4CAF50)
...
def _earliest(asset: str, tf: str, source: str = "gate") -> int | None:
    con = db(); row = con.execute("SELECT MIN(ts) FROM market_bars WHERE source=? AND asset=? AND tf=?", (source, asset, tf)).fetchone(); con.close(); return int(row[0]) if row and row[0] is not None else None
...
async def backfill_one(asset: str, tf: str) -> dict[str, Any]:
    ...
    added, source_used, error = 0, "gate", None
    ...
    con = db(); con.execute("INSERT OR REPLACE INTO source_ledger(ts,source,feature_group,ok,quality,detail) VALUES(?,?,?,?,?,?)",
                            (int(time.time()), source_used, f"backfill:{asset}:{tf}", int(added > 0), 100.0 if added else 0.0, json.dumps({"added": added, "error": error}, ensure_ascii=False))); con.commit(); con.close()
    return {"done": end_ts <= START_TS, "asset": asset, "tf": tf, "added": added, "source": source_used, "error": error}
...
def _best_source(asset: str, tf: str) -> str | None:
    ...
    gate = next((r for r in rows if r[0] == "gate" and r[2] is not None and r[2] <= START_TS + 7 * 86400), None)
    return "gate" if gate else rows[0][0]
...
def generate_learning_samples(batch: int = 450) -> int:
    src15, src1h, src4h, src1d, srcbtc = _best_source("ETH", "15m"), _best_source("ETH", "1h"), _best_source("ETH", "4h"), _best_source("ETH", "1d"), _best_source("BTC", "1h")
    if not all((src15, src1h, src4h, src1d, srcbtc)): return 0
    m15, h1, h4, d1, btc = load_bars("ETH", "15m", src15), load_bars("ETH", "1h", src1h), load_bars("ETH", "4h", src4h), load_bars("ETH", "1d", src1d), load_bars("BTC", "1h", srcbtc)
    if min(map(len, (m15, h1, h4, d1, btc))) < 120: return 0
    ts15, ts1, ts4, tsd, tsb = [x["ts"] for x in m15], [x["ts"] for x in h1], [x["ts"] for x in h4], [x["ts"] for x in d1], [x["ts"] for x in btc]
    last_ts = int(get_state("last_learning_sample_ts", START_TS) or START_TS); start_i = max(100, bisect.bisect_right(ts15, last_ts))
    con = db(); store = ModelStore(con); learner = Learner(store); created = processed = 0; newest = last_ts
    from adaptive_engine import baseline_strategy_scores
    for i in range(start_i, len(m15) - 25):
        ...
        regime = detect_regime(d1s, h4s, h1s); features = build_features(m15s, h1s, btcs, regime, {"source_agreement_bps": 10.0}); candidates = baseline_strategy_scores(features, regime)
        for strategy, meta in candidates.items():
            if meta["baseline"] < 0.22: continue
            success, pnl_r, mfe_r, mae_r = learner.outcome(m15, i, meta["direction"], 24)
            store.add_sample({"ts": ts, "strategy": strategy, "direction": meta["direction"], "regime": regime["regime"], "phase": regime["phase"],
                              "features": features, "success": success, "pnl_r": pnl_r, "mfe_r": mfe_r, "mae_r": mae_r, "source_quality": 82.0 if src15 == "gate" else 74.0}); created += 1
...
async def learning_tick() -> None:
    con = db(); progress = bootstrap_progress(con); con.close(); chosen = None
    ...
    if chosen: backfill_result = await backfill_one(*chosen)
        ...
        samples = generate_learning_samples()
        if samples and time.time() - int(get_state("last_train_ts", 0) or 0) > 45 * 60: train_if_due(force=True)
    ...
    con.close(); state["learning"] = {"progress": progress, "backfill": backfill_result, "sample_counts": counts, "champions": champions,
                                      "learning_order": ["1D/4H", "1H/30M", "15M/5M", "live/post-exit"]}
...
app = FastAPI(title="ETH Adaptive Short-Term Engine", version="4.0.0", lifespan=lifespan)
...
def mark_filled(signal_id: str, price: float | None = None) -> dict[str, Any]:
    ...
    fill = float(price or row["entry"]); payload = json.loads(row["payload"]); payload["manual_fill_price"] = fill
    con.execute("UPDATE signals SET status='OPEN',filled_at=?,updated_at=?,payload=? WHERE signal_id=?", (int(time.time()), int(time.time()), json.dumps(payload, ensure_ascii=False), signal_id)); con.commit(); con.close(); return {"ok": True, "signal_id": signal_id, "fill_reference": fill}
